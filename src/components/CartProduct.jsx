...
function CartProduct(props){
    ...
    return (
        ...
        <ClearIcon className="clear-icon" onClick={() => props.deleteItem(props.id)}/>
            ...
            <div className="flex-for-cart">
            <div className="priceFlex">
            <div className="cart-title"><h2>{amount +"x "}</h2><h3> {props.title }</h3></div>
            <FloatingLabel
                controlId="floatingInput"
                label="Set Amount"
                
            >
            <Form.Control className="inputCart"  onChange={handleChange}  type="number" min="1" placeholder="Amount" />
            </FloatingLabel>
            </div>
            <div className="priceFlex">
